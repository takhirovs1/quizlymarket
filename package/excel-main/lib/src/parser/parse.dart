...
class Parser {
      ...
      document.findAllElements('cellXfs').forEach((node1) {
        node1.findAllElements('xf').forEach((node) {
          ...
          if (fontId < fontList.length) {
            ...
            /// Checking for font Size.
            String? _size = _nodeChildren(font, 'sz', attribute: 'val') as String;
            if (_size != null) {
            ...
            if (_italic != null && _italic is bool && _italic) {
              isItalic = true;
            }
  ...
  dynamic _nodeChildren(XmlElement node, String child, {var attribute}) {
    var ele = node.findElements(child) as List<XmlElement>;
    if (ele.isNotEmpty) {
      if (attribute != null) {
        var attr = ele.first.getAttribute(attribute as String);
        if (attr != null) {
          return attr;
        }
  ...
  int _getFontIndex(var node, String text) {
    String? applyFont = node.getAttribute(text)?.trim() as String?;
    if (applyFont != null) {
      try {
